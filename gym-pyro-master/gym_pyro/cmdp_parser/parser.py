...
CMDP = namedtuple(
    'CMDP',
    'discount, values, confounders, states, actions, U, start, T, R, reset',
)
...
class CMDP_Parser:
    ...
    def p_cmdp(self, p):
        """ cmdp : preamble starting structure """
        self.cmdp = CMDP(
    ...
    def p_preamble(self, p):
        ...
        self.R = np.zeros(
            (self.nconfounders, self.nactions, self.nstates, self.nstates)
        )
    ...
    def p_preamble_confounders_N(self, p):
        """ preamble_item : CONFOUNDERS COLON INT """
        N = p[3]
        self.confounders = tuple(range(N))
        self.nconfounders = N

    def p_preamble_confounders_names(self, p):
        """ preamble_item : CONFOUNDERS COLON id_list """
        idlist = p[3]
        self.confounders = tuple(idlist)
        self.nconfounders = len(idlist)
    ...
    def p_starting(self, p):
        """ starting : starting_list """
        pass

    def p_starting_list(self, p):
        """ starting_list : starting_list confounding
                          | starting_list start
                          | confounding
                          | start
                          | """
    ...
    def p_confounding_uniform(self, p):
        """ confounding : U COLON UNIFORM """
        self.U = np.full(self.nconfounders, 1 / self.nconfounders)
    ...
    def p_confounder_idx(self, p):
        """ confounder : INT """
        p[0] = p[1]

    def p_confounder_id(self, p):
        """ confounder : ID """
        p[0] = self.confounders.index(p[1])

    def p_confounder_all(self, p):
        """ confounder : ASTERISK """
        p[0] = slice(None)
    ...
    def p_structure_t_as_dist(self, p):
        ...
        if not np.isclose(pmsum, 1.0):
            raise ParserError(
                f'Transition distribution (action={a}, '
    ...
    def p_structure_t_a_dist(self, p):
        ...
        if not np.isclose(pm.sum(axis=1), 1.0).all():
            raise ParserError(
                f'Transition state distribution (action={a}) is '
                'not normalized;'
            )
        self.T[a] = pm
    ...
    def p_structure_r_ass(self, p):
        """ structure_item : R COLON confounder COLON action COLON state COLON state number """
        u, a, s0, s1, r = p[3], p[5], p[7], p[9], p[10]
        self.R[u, a, s0, s1] = r

    def p_structure_r_as(self, p):
        """ structure_item : R COLON confounder COLON action COLON state nmatrix """
        u, a, s0, r = p[3], p[5], p[7], p[8]
        self.R[u, a, s0] = r
